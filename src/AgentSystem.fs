...
module AgentSystem = 
    ...
    type MailboxProcessor<'a> with 
        ...
        static member public SpawnAgent<'b>(messageHandler :'a->'b->'b, initialState : 'b, ?timeout:'b -> int, 
                                            ...
                                            ?errorHandler:Exception -> 'a option -> 'b -> AfterError<'b>) : MailboxProcessor<'a> = 
        ...
        static member public SpawnWorker(messageHandler,  ?timeout, ?timeoutHandler,?errorHandler) = 
            ...
            MailboxProcessor.SpawnAgent((fun msg _ -> messageHandler msg; ()), (), timeout, timeoutHandler, 
    ...
    type AsyncResultCell<'T>() = 
        ...
        member __.RegisterResult r = source.SetResult(r) 
        member __.AsyncWaitResult = 
    ...
    type WorkQueue() = 
        ...
        member __.Queue (f) = workQueue <-- f 
        member __.QueueWithTask f : Task<'T> = 
            let source = new TaskCompletionSource<_>() 
            workQueue <-- (fun () -> f() |> source.SetResult) 
            source.Task 
        member __.QueueWithAsync (f:unit -> 'T) : Async<'T> = 
            let result = new AsyncResultCell<'T>() 
            workQueue <-- (fun () -> f() |> result.RegisterResult ) 
            result.AsyncWaitResult 
        member __.Restart () = workQueue <-! Restart 
        member __.Stop () = workQueue <-! Stop 
        member __.SetErrorHandler(h) = 
            let managerF = fun (_, name:string, ex:Exception, _, _, _) -> h name ex                              
            let manager = spawnWorker managerF 
            workQueue <-! SetManager manager 
        member __.SetName(name) = workQueue <-! SetName(name) 
        member __.SetQueueHandler(g) = workQueue <-! SetWorkerHandler(g) 
        member __.SetTimeoutHandler(timeout, f) = workQueue <-! SetTimeoutHandler(timeout, f) 
